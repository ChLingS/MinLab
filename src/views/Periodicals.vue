<template>
    <div class="container">
        ...
        <div class="card-list">
            <div v-for="(patent, index) in visiblePeriodicals" :key="index"
                style="margin-top: 2rem;"
            v-slideln>
                <cardBlog :title="patent.title" 
                          :date="patent.date" 
                          :nameOfjournal="patent.periodical"
                          :text="patent.abstract"
                          :imageUrl="patent.image_url"
                          :paper_url="patent.paper_url" />
            </div>
        </div>
        <div v-if="periodicals.length > visiblePeriodicals.length" class="load-more">
...
import { usePeriodicals } from '@/Hooks/getPeriodicalData';

const { periodicals } = usePeriodicals();

const visibleCount = ref(10);

const visiblePeriodicals = computed(() => {
    return periodicals.value.slice(0, visibleCount.value);
});
